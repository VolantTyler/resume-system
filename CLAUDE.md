...
- Two project skills in `.claude/skills/` cover the repeated multi-step workflows and carry the conventions in detail: `process-intake` (any write into `data/` — YAML shapes, ID prefixes, bullet variants, claim-safety) and `apply-to-job` (JD → tailor → gap interview → fit analysis → cover letter). Prefer them over re-deriving the steps.
