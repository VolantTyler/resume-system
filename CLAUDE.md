...
- Project skills in `.claude/skills/` cover the repeated multi-step workflows and carry the conventions in detail: `process-intake` (any write into `data/` — YAML shapes, ID prefixes, bullet variants, claim-safety), `apply-to-job` (JD → tailor → gap interview → fit analysis → cover letter), and `resume-tailor-verify` (re-checking a draft's claims against source records). Prefer them over re-deriving the steps.
- **Keep skills small and single-purpose, and chain them for complex tasks** rather than growing one skill to cover a whole workflow. A skill should do one job well and hand off — `apply-to-job` calls into `resume-tailor-verify` for the verification pass instead of absorbing it, and both defer to `process-intake` for writes into `data/`. When a skill starts needing "and also" in its description, that's the signal to split it and chain instead.
